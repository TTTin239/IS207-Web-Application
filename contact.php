<?php
		include 'inc/header.php';
?>

 <div class="main">
    <div class="content">
    	<div class="support">
  				<img src="web/images/contact.png" alt="" />
  			<div class="clear"></div>
  		</div>
    	<div class="section group">
				<div class="col span_2_of_3">
				  <div class="contact-form">
				  	<h2 style="text-align: center;">Kết nối với chúng tôi</h2>
					    <form>
					    	<div>
						    	<span><label>TÊN</label></span>
						    	<span><input type="text" value=""></span>
						    </div>
						    <div>
						    	<span><label>E-MAIL</label></span>
						    	<span><input type="text" value=""></span>
						    </div>
						    <div>
						     	<span><label>SỐ ĐIỆN THOẠI</label></span>
						    	<span><input type="text" value=""></span>
						    </div>
						    <div>
						    	<span><label>Ý KIẾN</label></span>
						    	<span><textarea> </textarea></span>
						    </div>
						   <div>
						   		<span><input type="submit" value="GỬI" style="margin-right: 340px;"></span>
						  </div>
					    </form>
				  </div>
  				</div>
				<div class="col span_1_of_3">
      			<div class="company_address">
				     	<h2 style="text-align:center;">Thông tin liên lạc</h2>
						    	<p>Văn phòng chính: Tòa nhà Viettel, 285 Cách Mạng Tháng 8, Phường 12, Quận 10, Thành phố Hồ Chí Minh.</p>
						   		<p>Văn phòng: 52 Út Tịch, Phường 4, Quận Tân Bình, Thành Phố Hồ Chí Minh.</p>
						   		<p>Tổng đài hỗ trợ: 19001221</p>
				 	 			<p>Email: marketing@watchstore.vn</p></p>
				   </div>
				 </div>
			  </div>    	
    </div>
 </div>

<?php
		include 'inc/footer.php';
?>